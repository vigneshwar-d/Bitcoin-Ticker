...
//  BitcoinTicker
//
//  Created by Vigneshwar Devendran on 23/01/2018.
...
class ViewController: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        ...
        selectedPlaceInArray = currencySymbol[row]
        print(finalURL)
...
//    
    func updateCoinData(json : JSON) {
        ...
        if let coinResult = json["ask"].double {
            print(coinResult)
            bitcoinPriceLabel.text = "\(selectedPlaceInArray)\(coinResult)"
        }
        else{
            bitcoinPriceLabel.text = "API Issues"
        }
}
